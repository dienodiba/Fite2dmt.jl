# Dieno Diba 2024
# FITE2DMT
# Inversion of rhoa phase

function InversionRhoaPhase(filedat::String, filem0::String, filetopo::String, filestg::String)
    ...
    lambda,max_iter,outroot,fix_el,damping,hor2ver,static,nodata,wg_rte,wg_pte,wg_rtm,wg_ptm,ef_zte,ef_ztm = ReadInversionSettingParametersRhoaPhase(filestg)

    # Read observation data of rhoa, phase
    nst,nper,sta,period,rhe_o,ere_o,phe_o,epe_o,rhh_o,erh_o,phh_o,eph_o,dobs,eobs,N,W,V,T,nnzN,idd1,idd4 = ReadObservedDataRhoaPhase(filedat,nodata,wg_rte,wg_pte,wg_rtm,wg_ptm,ef_zte,ef_ztm)
	...
	Dh = ConstructStiffnessMatrixTM(nel_h,nno_h,el2no_h,no2yz_h,rho_h,Area,eh2ee)
        ...
        for idp = 1:nper
            ...
            dcale1,ce1 = CalculateResponseFunctionsTERhoaPhase(idp,dcale1,nno,nst,no2yz,ns1e,ns2e,nbse,omega,mu0,ve)

            # Construct TE mode jacobian matrix: rhoa, phase, tipper
            ce1[noin_e,:] = dcDe\ce1[noin_e,:]
            Aheg1 = ConstructJacobianTERhoaPhase(idp,nst,Aheg1,M,el2no,m2ee,omega,mu0,Area,rho,ce1,ve)

        end

        # Merge jacobian matrices
        A = MergeJacobianRhoaPhase(N,M,nper,nst,Aheg1,Ahhg)

        # Calculate the data misfit, model roughness, and objective function
        dcal,RMSd1,RMSd4,Phid1,Phid4,Phi,RMS,Stab,Psi = CalculateObjectiveFunctionRhoaPhase(RMSd1,RMSd4,Phid1,Phid4,Phi,RMS,Stab,Psi,N,itr,nper,nst,dcale1,dcalh,dobs,W,T,lambda,m,K,idd1,idd4)
    ...
    # Export Log File
    ExportLogFileRhoaPhase(outroot,filedat,filem0,filestg,filetopo,epoch,idd1,idd4,M,t1,t2,Phi,Psi,Stab,lambda,RMS,RMSd1,RMSd4,runtime)
    # Export the final model
    ExportFinalModelResistivity(outroot,nel,nno,el2no,no2yz,frho)
    #Export model responses
    ExportFinalModelRhoaPhase(outroot,nper,nst,sta,dpre,period,rhe_o,ere_o,phe_o,epe_o,rhh_o,erh_o,phh_o,eph_o)
