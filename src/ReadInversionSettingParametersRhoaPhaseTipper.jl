...
function ReadInversionSettingParametersRhoaPhaseTipper(filestg::String)
    ...
    wg_rte = stg[only(findall(x->x=="USE_RTE",stg[:,1]))+1,1]
    wg_pte = stg[only(findall(x->x=="USE_PTE",stg[:,1]))+1,1]
    wg_rtm = stg[only(findall(x->x=="USE_RTM",stg[:,1]))+1,1]
    wg_ptm = stg[only(findall(x->x=="USE_PTM",stg[:,1]))+1,1]
    wg_tyr = stg[only(findall(x->x=="USE_TYR",stg[:,1]))+1,1]
    wg_tyi = stg[only(findall(x->x=="USE_TYI",stg[:,1]))+1,1]
    ...
    return lambda,max_iter,outroot,fix_el,damping,hor2ver,static,nodata,wg_rte,wg_pte,wg_rtm,wg_ptm,wg_tyr,wg_tyi,ef_zte,ef_ztm,ef_tzy
