# Dieno Diba 2024
# FITE2DMT
# Export rhoa phase tipper of the final inversion model

function ExportFinalModelRhoaPhaseTipper(outroot::Any, nper::Integer, nst::Integer, sta::Matrix, dpre::Vector, period::Vector, rhe_o::Matrix, ere_o::Matrix, phe_o::Matrix, epe_o::Matrix, rhh_o::Matrix, erh_o::Matrix, phh_o::Matrix, eph_o::Matrix, rtz_o::Matrix, ert_o::Matrix, itz_o::Matrix, eit_o::Matrix)
    ...
    for ids = 1:nst
        ...
        @printf(fid,"y(km) = %f\n",sta[ids,1]*1e-3)
        @printf(fid,"z(km) = %f\n",sta[ids,2]*1e-3)
        print(fid,"      period(s) roe_obs(Ohm.m) roe_cal(Ohm.m) roe_err(Ohm.m) roh_obs(Ohm.m) roh_cal(Ohm.m) roh_err(Ohm.m)   phe_obs(deg)   phe_cal(deg)   phe_err(deg)   phh_obs(deg)   phh_cal(deg)   phh_err(deg)       tzyr_obs       tzyr_cal       tzyr_err       tzyi_obs       tzyi_cal       tzyi_err\n")
        for idp = 1:nper
            ...
            @printf(fid,"%15.5e%15.5e%15.5e",phh_o[idp,ids],phh_c[idp,ids],eph_o[idp,ids])
